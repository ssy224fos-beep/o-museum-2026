...
トップページの「Sランク スタンプラリー」から、Sランク作品だけをまとめた印刷用カードを開けます。フロア、展示室番号、作品番号、作品名、作者、サムネイル、スタンプ欄を掲載し、A4縦で1ページ6作品です。ランクを変更すると掲載作品も自動で更新されます。

ブラウザ上ではスタンプ欄をタップして獲得状態を切り替えられます。進捗表示と「すべて／未獲得／獲得済み」の絞り込みがあり、記録は端末のブラウザ内に保存されます。印刷時はブラウザ上の獲得状態を反映せず、空のスタンプ欄を表示します。

サムネイルは `data/stamp-images.js` でWikimedia Commonsのファイル名と対応させています。画像出典ページから各ファイルのライセンスを確認できます。
